...
function CustMgmt() { 
  ...
  this.getContact = function(id) {
    return $.ajax({
      type: 'GET',
      url:  '/contacts/'+id+'?projection=complete'
    });
  };
  this.saveContact = function(contact) {
    console.log('saveContact '+JSON.stringify(contact)+' ...');
    var id = contact._links === undefined ? undefined : (
        contact._links.self.href.indexOf('?') == -1 ? contact._links.self.href : contact._links.self.href.substr(0,contact._links.self.href.indexOf('?')-1)
    );
    return $.ajax({
...
//        if (jqXHR.status == 201) ractive.data.contacts.push(contact);
//        if (jqXHR.status == 204) ractive.splice('contacts',contactIdx,1,contact);
        console.info('All saved');
      },
      error: errorHandler = function(jqXHR, textStatus, errorThrown) {
        console.error("Bother! Something has gone wrong: "+textStatus+':'+errorThrown);
      }
    });
  }
}
...
/*var ractive = new AuthenticatedRactive({
  ...
  fetch: function () {
    console.log('fetch...');
    $.getJSON('/'+ractive.data.tenant.name+'/contacts/?projection=complete',  function( data ) {
...
//	  this.fetch();
  ...
  save: function () {
    ...
    ractive.data.suspendSaveObserver = true;
    ...
    ractive.data.suspendSaveObserver = false;
