...
@Entity
@NamedEntityGraph(name = "accountOnly", attributeNodes = {
    @NamedAttributeNode("customFields"), })
@NamedEntityGraph(name = "accountWithAll", attributeNodes = {
    @NamedAttributeNode("activities"), @NamedAttributeNode("customFields"),
    @NamedAttributeNode("notes"), @NamedAttributeNode("documents") })
@Table(name = "OL_ACCOUNT")
@SecondaryTable(name = "OL_ACCOUNT_CUSTOM", pkJoinColumns = @PrimaryKeyJoinColumn(name = "account_id"))
@Data
...
public class Account extends Auditable<String> implements Serializable {
    ...
    public String toCsv() {
        StringBuilder sb = new StringBuilder().append(String.format(
                ...
                id, name, companyNumber == null ? "" : companyNumber,
                ...
                phone1 == null ? "" : phone1, phone2 == null ? "" : phone2,
                owner == null ? "" : owner, stage == null ? "" : stage,
                ...
                description == null ? ""
                        : CsvUtils
                                .quoteIfNeeded(description.replace('"', '\'')),
                incorporationYear == null ? "" : incorporationYear,
                noOfEmployees == null ? "" : noOfEmployees, tenantId,
                firstContact, lastUpdated, getConsolidatedNotes(),
    ...
    public String toString() {
        return String.format(
                "Account [id=%s, name=%s, companyNumber=%s, aliases=%s, businessWebsite=%s, shortDesc=%s, description=%s, incorporationYear=%s, noOfEmployees=%s, tenantId=%s, firstContact=%s, lastUpdated=%s, customFields=%s]",
                id, name, companyNumber, aliases, businessWebsite, shortDesc,
                description, incorporationYear, noOfEmployees, tenantId,
                firstContact, lastUpdated, customFields);
    }
}
