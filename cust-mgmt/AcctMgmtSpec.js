...
describe("Account management", function() {
  ...
  it("adds a contact to the new account", function(done) {
    $rh.ajax({
      ...
      success: completeHandler = function(data, textStatus, jqXHR) {
        ...
        var accountContactLink = $rh.uri(account);
        $rh.ajax({
          url: accountContactLink+'/contact',
          type: 'PUT',
          contentType: 'text/uri-list',
          data: location,
          success: function(data, textStatus, jqXHR) {
            console.log('linked contact: '+location+' to '+accountContactLink);
            done();
          }
        });
      }
  ...
  it("fetches complete account inc. child entities and check all fields are correct", function(done) {
    $rh.getJSON($rh.tenantUri(account),  function( data ) {
      ...
      expect(data.activities[2].content).toMatch(/Linked contact [0-9]* to account [0-9]*/);
