...
public class OAuthAuthenticationExample {
		...
		if (line.hasOption(HELP_OPTION)) {
			...
			try {
				...
				System.out.println("Expiration time: "
						+ requestToken.getExpirationTime());
	...
	private static void printHelp(Options options) {
		int width = 80;
		String syntax = OAuthAuthenticationExample.class.getName()
				+ " <options>";
		String header = MessageFormat.format(
				"\nBoth the -{0} and -{1} options are required.",
				CONSUMER_KEY_OPTION, CONSUMER_SECRET_OPTION);
		new HelpFormatter().printHelp(width, syntax, header, options, null,
				false);
	}
