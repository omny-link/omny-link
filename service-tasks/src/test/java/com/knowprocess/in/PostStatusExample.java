...
public class PostStatusExample {
	 ...
	 * Status Text
	 */
	private static final String STATUS_TEXT_OPTION = "status";
	...
	private static void processCommandLine(CommandLine line, Options options) {
		if (line.hasOption(HELP_OPTION)) {
			...
			if (line.hasOption(STATUS_TEXT_OPTION)) {
				final String status = line.getOptionValue(STATUS_TEXT_OPTION);
				client.updateCurrentStatus(status);
				System.out
						.println("Your status has been posted. Check the LinkedIn site for confirmation.");
			} else if (line.hasOption(DELETE_OPTION)) {
	...
	private static void printHelp(Options options) {
		int width = 80;
		String syntax = PostStatusExample.class.getName() + " <options>";
		String header = MessageFormat
				.format("\nThe -{0}, -{1}, -{2} and -{3} options are required. All others are optional.",
		...
		String footer = MessageFormat.format(
				"\nYou can specify only one of -{0} or -{1} options.",
				STATUS_TEXT_OPTION, DELETE_OPTION);
