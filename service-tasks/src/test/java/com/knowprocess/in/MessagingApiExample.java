...
public class MessagingApiExample {
	...
	private static void processCommandLine(CommandLine line, Options options) {
		if (line.hasOption(HELP_OPTION)) {
			printHelp(options);
		} else if (line.hasOption(CONSUMER_KEY_OPTION)
				&& line.hasOption(CONSUMER_SECRET_OPTION)
				&& line.hasOption(ACCESS_TOKEN_OPTION)
				&& line.hasOption(ACCESS_TOKEN_SECRET_OPTION)
				&& line.hasOption(SUBJECT_OPTION)
				&& line.hasOption(MESSAGE_OPTION)) {
			final String consumerKeyValue = line
					.getOptionValue(CONSUMER_KEY_OPTION);
			final String consumerSecretValue = line
					.getOptionValue(CONSUMER_SECRET_OPTION);
			final String accessTokenValue = line
					.getOptionValue(ACCESS_TOKEN_OPTION);
			final String tokenSecretValue = line
					.getOptionValue(ACCESS_TOKEN_SECRET_OPTION);
			...
			if (line.hasOption(ID_OPTION)) {
				...
				System.out.println("Sending message to users with ids:"
						+ idValue);
				client.sendMessage(Arrays.asList(idValue.split(",")), subject,
						message);
				System.out
				...
				client.sendMessage(Collections.singletonList("~"), subject,
						message);
				System.out
						.println("Your message has been sent. Check the LinkedIn site for confirmation.");
			}
	...
	private static Options buildOptions() {
		...
		Option accessTokenSecret = OptionBuilder
				.create(ACCESS_TOKEN_SECRET_OPTION);
	...
	private static void printHelp(Options options) {
		...
		String header = MessageFormat
				.format("\nThe -{0}, -{1}, -{2} , -{3}, -{4}, -{5} options are required. The -{6} option is optional.",
						CONSUMER_KEY_OPTION, CONSUMER_SECRET_OPTION,
						ACCESS_TOKEN_OPTION, ACCESS_TOKEN_SECRET_OPTION,
						SUBJECT_OPTION, MESSAGE_OPTION, ID_OPTION);
		String footer = MessageFormat
				.format("\nIf you do not specify -{0} option, the message is sent to the current user. You can specify multiple ids separated by comma.",
						ID_OPTION);
