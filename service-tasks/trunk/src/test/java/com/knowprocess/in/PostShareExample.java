...
public class PostShareExample {
	...
	private static void processCommandLine(CommandLine line, Options options) {
		if (line.hasOption(HELP_OPTION)) {
			...
			if (line.hasOption(SHARE_TEXT_OPTION)) {
				...
				client.postShare(status, "Testing share.",
						line.getOptionValue(URL_OPTION), null,
						VisibilityType.CONNECTIONS_ONLY);
				System.out
						.println("Your share has been posted. Check the LinkedIn site for confirmation.");
			}
		} else {
			printHelp(options);
		}
	...
	private static void printHelp(Options options) {
		...
		String footer = MessageFormat.format(
				"\nYou should specify both of -{0} or -{1} options.",
				SHARE_TEXT_OPTION, URL_OPTION);
