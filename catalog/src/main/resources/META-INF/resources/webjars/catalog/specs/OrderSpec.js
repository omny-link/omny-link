...
describe("Product catalogue", function () {
  ...
  it("creates a new stock item", function (done) {
    $rh.ajax({
      ...
      success: function (data, textStatus, jqXHR) {
        ...
        complexOrder.orderItems[0].stockItemId = parseInt(
          location.substring(location.lastIndexOf("/") + 1)
        );
        complexOrder.orderItems[1].stockItemId = parseInt(
          location.substring(location.lastIndexOf("/") + 1)
        );
        done();
      },
    ...
    $rh.ajax({
      ...
      success: function (data, textStatus, jqXHR) {
        ...
        $rh.getJSON($rh.tenantUri(order), function (data) {
          ...
          expect(data.customFields.specialInstructions).toEqual(
  ...
  it("fetches complete order inc. child entities and check all fields are correct", function (done) {
    ...
    $rh.getJSON($rh.tenantUri(order), function (data) {
      ...
      expect(data.feedback.customFields.refusalReason).toEqual(
        feedback.customFields.refusalReason
      );
      expect(data.feedback.customFields.refusalDate).toEqual(
        feedback.customFields.refusalDate
      );
  ...
  it("fetches complete order inc. child entities BY CONTACT and check all fields are correct", function (done) {
    ...
    $rh.ajax({
      ...
      success: function (data, textStatus, jqXHR) {
        ...
        // .. then an order ...
        orderWithContact.contactId = location.substring(
          ...
          success: function (data, textStatus, jqXHR) {
            ...
            $rh.ajax({
              ...
              success: function (data, textStatus, jqXHR) {
                ...
                console.log(
                  "tenantUri: " +
                    $rh.tenantUri(orderWithContact) +
                    ", contactId: " +
                    orderWithContact.contactId
                );
                console.log(
                  "tenantUri: " +
                    $rh.localId(orderWithContact) +
                    ", contactId: " +
                    orderWithContact.contactId
                );
                $rh.getJSON(
                  "/" +
                    tenantId +
                    "/orders/findByContacts/" +
                    orderWithContact.contactId,
                  function (data) {
                    expect(data.length).toEqual(1);
                    data = data[0];

                    expect($rh.localId(data)).toEqual(
                      $rh.localId(orderWithContact)
                    );
                    ...
                    expect(data.customFields).toBeDefined();
                    expect(data.customFields.specialInstructions).toEqual(
                      orderWithContact.customFields.specialInstructions
                    );

                    expect(data.feedback.type).toEqual(feedback.type);
                    expect(data.feedback.description).toEqual(
  ...
  it("fetches the complex order inc. order items and check all fields are correct", function (done) {
    ...
    $rh.getJSON($rh.tenantUri(complexOrder), function (data) {
      ...
      expect(data.orderItems[0].price).toEqual(
        complexOrder.orderItems[0].price
      );
      expect(data.orderItems[0].status).toEqual(
        complexOrder.orderItems[0].status
      );
      expect(data.orderItems[0].dueDate).toEqual(
        complexOrder.orderItems[0].dueDate
      );
      expect(data.orderItems[0].owner).toEqual(
        complexOrder.orderItems[0].owner
      );
      expect(data.orderItems[0].stockItemId).toBeDefined();
      expect(data.orderItems[0].stockItemId).toEqual(
        complexOrder.orderItems[0].stockItemId
      );
      expect(data.orderItems[0].customFields.controlledGoods).toEqual(
        complexOrder.orderItems[0].customFields.controlledGoods
      );

      expect(data.orderItems[1].price).toEqual(
        complexOrder.orderItems[1].price
      );
      expect(data.orderItems[1].status).toEqual(
        complexOrder.orderItems[1].status
      );
      expect(data.orderItems[1].dueDate).toEqual(
        complexOrder.orderItems[1].dueDate
      );
      expect(data.orderItems[1].owner).toEqual(
        complexOrder.orderItems[1].owner
      );
      expect(data.orderItems[1].stockItemId).toBeDefined();
      expect(data.orderItems[1].stockItemId).toEqual(
        complexOrder.orderItems[1].stockItemId
      );
      expect(data.orderItems[1].customFields.signatureRequired).toEqual(
        complexOrder.orderItems[1].customFields.signatureRequired
      );
  ...
  it("deletes the last item of the complex order", function (done) {
    $rh.ajax({
      url:
        $rh.tenantUri(complexOrder) + "/order-items/" + additionalOrderItem.id,
      type: "DELETE",
      contentType: "application/json",
      success: function (data, textStatus, jqXHR) {
        expect(jqXHR.status).toEqual(204);
