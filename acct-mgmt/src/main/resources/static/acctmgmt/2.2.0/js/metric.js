...
var ractive = new BaseRactive({
  ...
  add: function () {
    ...
    var metric = { account: {}, author:$auth.getClaim('sub'), tenantId: ractive.get('tenant.id'), url: undefined };
  ...
  find: function(metricId) {
    ...
    $.each(ractive.get('metrics'), function(i,d) {
      if (metricId.endsWith(ractive.id(d))) {
        c = d;
      }
    });
    return c;
...
//      console.log('ready to save metric'+JSON.stringify(tmp)+' ...');
  ...
  select: function(metric) {
    ...
    if (metric.owner == undefined || metric.owner == '') metric.owner = $auth.getClaim('sub');
