...
var BaseRactive = Ractive.extend({
  ...
  addDataList: function (d, data) {
    if (d.name == undefined) {
      console.warn(
        'Data list with selector "' +
          d.ref +
          '" has no name, please add one to use as a datalist',
      );
      return;
    }
    ...
    if (data == null) {
      console.error(
        "No data for datalist: " + d.name + ", please fix configuration",
      );
    } else {
    ...
    if (activities.length > 0) {
      ...
      for (var idx in activities) {
        if (activities[idx].type == "email") {
          ...
          if (idx == 0)
            ractive.set(
              "current.timeSinceEmail",
              new Date().getTime() - activities[idx].occurred,
            );
        }
      }
  ...
  applyAccessControl: function () {
    ...
    if (
      ...
      ractive.get("tenant.access.readonly")
    ) {
  ...
  applyBranding: function () {
    ...
    if (tenant != undefined) {
      if (ractive.get("tenant.theme.cssUrl") !== undefined) {
        $("head").append(
          '<link href="' +
            ractive.get("tenant.theme.cssUrl") +
            '" rel="stylesheet">',
        );
      }
      if (ractive.get("tenant.theme.logoUrl") != undefined) {
        $(".navbar-brand")
          ...
          .append(
            '<img src="' +
              ractive.get("tenant.theme.logoUrl") +
              '" alt="logo"/>',
          );
      }
      ...
      if (ractive.brandingCallbacks != undefined)
        ractive.brandingCallbacks.fire();
    }
  },
  daysAgo: function (noOfDays) {
    return new Date(new Date().setDate(new Date().getDate() - noOfDays))
      .toISOString()
      .substring(0, 10);
  },
  download: function (entityName) {
    ...
    $.ajax({
      ...
      success: function (data) {
        console.info(
          "CSV received, first record: " +
            data.substring(0, data.indexOf("\n")),
        );
        var entityLabel =
          ractive.get("tenant.strings." + entityName.toCamelCase()) == undefined
            ? entityName
            : ractive.get("tenant.strings." + entityName.toCamelCase());
        try {
          var blob = new Blob([data], { type: "text/csv" });
          ractive.downloadUri(URL.createObjectURL(blob), entityLabel + ".csv");
        } catch (e) {
          ractive.showError(
            "Your browser does not support downloading large files, please try a more modern one.",
          );
        }
      },
    });
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $.each(ractive.get("stockCategories"), function (i, d) {
          $("datalist#stockCategories").append(
            '<option value="' + d.name + '">' + d.name + "</option>",
          );
        });
  ...
  fetchStockItems: function () {
    ...
    $.ajax({
      dataType: "json",
      url:
        ractive.getServer() + "/" + ractive.get("tenant.id") + "/stock-items/",
      crossDomain: true,
      success: function (data) {
        if ("_embedded" in data) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        data.forEach(function (t) {
          ...
          t.businessKey =
            ractive.get("current.name") == undefined
              ? ractive.get("current.fullName")
              : ractive.get("current.name");
  ...
  filter: function (filter) {
    ...
    $(".omny-dropdown.dropdown-menu li:nth-child(" + filter.idx + ")").addClass(
      "selected",
    );
  ...
  getProfile: function () {
    ...
    if (username) {
      ...
      $(".profile-img")
        .empty()
        .append(
          '<img class="img-rounded" src="//www.gravatar.com/avatar/' +
            ractive.hash(ractive.get("profile.email")) +
            '?s=34" title="' +
            ractive.get("profile.email") +
            '"/>',
        );
  ...
  getStockItemNames: function (order) {
    if (order == undefined) {
      console.warn(
        "ignoring attempt to find stockItem.names for undefined order",
      );
      return;
    }
    var stockItemIds = [];
    if (!ractive.get("tenant.features.orderItems")) {
      ...
      // tenant.features.orderItems == true
    ...
    console.warn(
      "  found " + stockItemIds.length + " stockItemIds: " + stockItemIds,
    );
    try {
      return stockItemIds
        .map((x) => Array.findBy("id", x, ractive.get("stockItems")).name)
        .join();
    } catch (e) {
      console.error(
        "  cannot find stockItemNames from " + stockItemIds + ", cause: " + e,
      );
    }
  },
  ...
  hasRoleLegacy: function (role) {
    ...
    if (this && this.get("profile")) {
      ...
      if (ractive.get("profile.groups") != undefined) {
        hasRole = ractive.get("profile.groups").filter(function (g) {
          return g.id == role;
        });
      }
  ...
  hideResults: function () {
    ...
    $("#" + entityName + "sTableToggle")
      .addClass("kp-icon-caret-right")
      .removeClass("kp-icon-caret-down");
  ...
  html2Pdf: function (html, fileName) {
    return fetch(ractive.getServer() + "/pdfs/" + fileName + ".pdf", {
      ...
      .then((data) =>
        ractive.saveAs(URL.createObjectURL(data), fileName + ".pdf"),
      );
  },
  ...
  initAbout: function () {
    $(".powered-by-icon")
      ...
      .append(
        '<img src="' +
          ractive.getServer() +
          '/images/icon/omny-link-icon.svg" alt="' +
          $env.appName +
          '">',
      );
  },
  initAutoComplete: function () {
    ...
    if (
      ractive.get("tenant.typeaheadControls") != undefined &&
      ractive.get("tenant.typeaheadControls").length > 0
    ) {
        ...
        if (d.url == undefined) {
          ...
          $.getJSON(url, function (data) {
            if (data == null || !Array.isArray(data)) {
              console.info(
                "No values for datalist: " +
                  d.name +
                  ", probably refreshing token",
              );
            } else {
              d.values = data;
              ractive.addDataList(d, d.values);
            ...
            if (jqXHR.status == 401)
              console.info(
                "No values for datalist: " +
                  d.name +
                  ", need to refresh token or login again",
              );
            else
              console.error(
                "No values for datalist: " +
  ...
  initEditor: function () {
    ...
    CKEDITOR.instances.curDescription.setData(
      ractive.get("current.description"),
    );
  },
  ...
  initShortKeys: function () {
    $("body").keypress(function (event) {
      if (
        ...
        return;
      switch (
        event.which // ref http://keycode.info/
      ) {
        case 13: // enter
      ...
      $(".initiator-img")
        ...
        .append(
          '<img class="img-rounded" src="' +
            (ractive.get("tenant.strings.botIcon")
              ? ractive.get("tenant.strings.botIcon")
              : ractive.getServer() + "/images/icon/omny-link-icon.svg") +
            '" width="34"/>',
        );
    } else {
  ...
  json2Html: function (obj) {
    ...
    $.each(Object.keys(obj), function (i, d) {
      if (
        ...
        // empty jakarta.json.JsonObject
        console.info("Supressing empty value " + d);
      } else if (typeof obj[d] == "object" && "valueType" in obj[d]) {
        // populated jakarta.json.JsonObject
        html +=
          '<li><label style="text-align:right;padding-right:10px">' +
          d.toLabel() +
          ':</label><span class="col-scroll">' +
          obj[d].string +
          "</span>";
  ...
  loadTenantConfig: function (tenant) {
    ...
    $.getJSON(
      "__CONFIG_SERVER_URL__/tenants/" + tenant + "/" + tenant + ".json",
      function (response) {
        ...
        ractive.set("saveObserver", true);
        if (ractive.tenantCallbacks != undefined)
          ractive.tenantCallbacks.fire();
      },
    );
  },
  logout: function () {
  ...
  rewrite: function (id) {
    ...
    if (
      ractive.get("server") != undefined &&
      ractive.get("server") != "" &&
      id.indexOf(ractive.get("server")) == -1
    ) {
      ...
      if (id.indexOf("://") == -1) {
        ...
        return (
          ractive.getServer() +
          id.substring(id.indexOf("/", id.indexOf("://") + 4))
        );
      }
    } else {
      return id;
  ...
  search: function (searchTerm) {
    ...
    setTimeout(function () {
      ...
      ractive.set(
        "searchMatched",
        $(".resultsSect .table tbody tr:visible").length,
      );
  ...
  localId: function (uriOrObj) {
    ...
    else if (typeof uriOrObj == "object" && "id" in uriOrObj)
      return "" + uriOrObj.id;
    else if (typeof uriOrObj == "object")
      return ractive.localId(ractive.uri(uriOrObj));
  ...
  showDisconnected: function (msg) {
    ...
    if ($("#connectivityMessages").length > 0) {
      ...
      $("body")
        .append(
          '<div id="connectivityMessages" class="alert-warning">' +
            msg +
            "</div>",
        )
        .show();
  ...
  showMessage: function (msg, additionalClass) {
    ...
    if (fadeOutMessages && additionalClass != "bg-danger text-danger")
      setTimeout(function () {
        $("#messages, .messages").fadeOut();
      }, ractive.EASING_DURATION * 10);
    else
      $("#messages, .messages").append(
        '<span class="text-danger pull-right glyphicon glyphicon-remove" onclick="ractive.hideMessage()"></span>',
      );
  },
  showReconnected: function () {
    ...
    if ($("#connectivityMessages:visible").length > 0) {
      ...
      $("body")
        .append(
          '<div id="connectivityMessages" class="alert-info">Reconnected</div>',
        )
        .show();
      setTimeout(function () {
  ...
  showSearchMatched: function () {
    ...
    ractive.set(
      "searchMatched",
      $("#" + entityName + "sTable tbody tr").length,
    );
  ...
  sortBy: function (array, column, asc) {
    ...
    return array.sort(function (a, b) {
      if (b[column] == undefined || b[column] == null || b[column] == "") {
        return a[column] == undefined || a[column] == null || a[column] == ""
          ? 0
          : -1;
      } else if (asc) {
  ...
  startCustomAction: function (key, label, object, form, businessKey) {
    console.log(
      "startCustomAction: " +
        key +
        (object == undefined ? "" : " for " + object.id),
    );
    var instanceToStart = {
    ...
    if (object != undefined) {
      var singularEntityName = ractive
        .entityName(object)
        .toCamelCase()
        .singular();
      instanceToStart.variables[singularEntityName + "Id"] = ractive.localId(
        ractive.uri(object),
      );
    }
    ...
    if (form == undefined || !form) {
      ...
      $("#submitCustomActionForm")
        .off()
        .on("click", ractive.submitCustomAction);
  ...
  submitCustomAction: function () {
    ...
    if (document.getElementById("customActionForm").checkValidity()) {
      if (
        ractive.get("instanceToStart.processDefinitionKeyOverride") != undefined
      )
        ractive.set(
          "instanceToStart.processDefinitionKey",
          ractive.get("instanceToStart.processDefinitionKeyOverride"),
        );
      var data = {
        processDefinitionKey: ractive.get(
          "instanceToStart.processDefinitionKey",
        ),
        businessKey:
          ractive.get("instanceToStart.label") +
          " to/for " +
          ractive.get("instanceToStart.businessKey"),
        name:
          ractive.get("instanceToStart.processDefinitionKey") +
          " " +
          ractive.localId(ractive.get("current")),
        variables: [],
        /*tenantId: ractive.get('tenant.id'),*/
      ...
      $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
          ...
          if (
            ...
            ractive.showMessage(
              'Started workflow "' + ractive.get("instanceToStart.label") + '"',
            );
          } else {
            ractive.showMessage(
              'Started workflow "' +
          ...
          if (
            document.location.href.endsWith("contacts.html") ||
            document.location.href.endsWith("accounts.html")
          ) {
            // refresh individual record
            setTimeout(
              ractive.select,
              ractive.EASING_DURATION * 5,
              ractive.get("current"),
            );
          } else {
            // refresh list
          ...
          if (ractive.customActionCallbacks != undefined)
            ractive.customActionCallbacks.fire(
              jqXHR.getResponseHeader("Location"),
            );
  ...
  tenantUri: function (entity, entityPath) {
    ...
    if (uri != undefined && uri.indexOf(ractive.get("tenant.id") + "/") == -1) {
      uri = uri.replace(
        entityPath,
        "/" + ractive.get("tenant.id") + entityPath,
      );
    }
    return uri;
  },
  toCsv: function (json, title, headings) {
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      for (var j = 0; j < propNames.length; j++) {
        try {
          ...
          console.error(
            "Fail to extract property " + propNames[j] + "from row " + i,
          );
        }
      }
    ...
    var fileName =
      title.replace(/ /g, "_") +
      new Date().toISOString().substring(0, 16).replace(/[T:]/g, "-");
  ...
  toggleResults: function () {
    ...
    var entityName = ractive.get("entityName");
    $("#" + entityName + "sTableToggle")
      .toggleClass("kp-icon-caret-down")
  ...
  uri: function (entity) {
    ...
    try {
      ...
      if (
        uri != undefined &&
        uri.indexOf(ractive.getServer()) == -1 &&
        uri.indexOf("//") != -1
      ) {
        uri =
          ractive.getServer() +
...
$(document).ready(function () {
  // Global jQuery AJAX prefilter to add JWT token to all requests
  $.ajaxPrefilter(function (options, originalOptions, jqXHR) {
    if (ractive.keycloak && ractive.keycloak.token) {
      if (!options.headers) {
        options.headers = {};
      }
      options.headers["Authorization"] = "Bearer " + ractive.keycloak.token;
    }
  });
  ...
  $(document).ajaxError(function (event, jqXHR, ajaxSettings, thrownError) {
    if (jqXHR.status === 401) {
      console.warn(
        "Authentication failed (401). Attempting to refresh token...",
      );
      try {
  ...
  if ($("#ajax-loader").length == 0)
    $("body").append(
      '<div id="ajax-loader"><img class="ajax-loader" src="/images/ajax-loader.gif" style="width:10%" alt="Loading..."/></div>',
    );
  ...
  ractive.observe("tenant", function (newValue, oldValue, keypath) {
    ...
    if (
      (oldValue == undefined || oldValue.id == "") &&
      newValue != undefined &&
      newValue.id != "" &&
      "fetch" in ractive
    ) {
    ...
    if (
      newValue != undefined &&
      newValue.accountFields != undefined &&
      newValue.accountFields.length > 0
    ) {
      for (let idx = 0; idx < newValue.accountFields.length; idx++) {
        if (
          newValue.accountFields[idx].type == "account" &&
          $("datalist#accounts").length == 0
        ) {
          if ("fetchAccounts" in ractive) ractive.fetchAccounts();
        } else if (
          newValue.accountFields[idx].type == "contact" &&
          $("datalist#contacts").length == 0
        ) {
    ...
    if (
      newValue != undefined &&
      newValue.contactFields != undefined &&
      newValue.contactFields.length > 0
    ) {
      for (let idx = 0; idx < newValue.contactFields.length; idx++) {
        if (
          newValue.contactFields[idx].type == "account" &&
          $("datalist#accounts").length == 0
        ) {
          if ("fetchAccounts" in ractive) ractive.fetchAccounts();
        } else if (
          newValue.contactFields[idx].type == "contact" &&
          $("datalist#contacts").length == 0
        ) {
          if ("fetchContacts" in ractive) ractive.fetchContacts();
        }
    ...
    if (
      newValue != undefined &&
      newValue.orderFields != undefined &&
      newValue.orderFields.length > 0
    ) {
      for (let idx = 0; idx < newValue.orderFields.length; idx++) {
        if (
          newValue.orderFields[idx].type == "account" &&
          $("datalist#accounts").length == 0
        ) {
          if ("fetchAccounts" in ractive) ractive.fetchAccounts();
        } else if (
          newValue.orderFields[idx].type == "contact" &&
          $("datalist#contacts").length == 0
        ) {
          if ("fetchContacts" in ractive) ractive.fetchContacts();
        }
    ...
    if (
      newValue != undefined &&
      newValue.orderItemFields != undefined &&
      newValue.orderItemFields.length > 0
    ) {
      for (let idx = 0; idx < newValue.orderItemFields.length; idx++) {
        if (
          newValue.orderItemFields[idx].type == "account" &&
          $("datalist#accounts").length == 0
        ) {
          if ("fetchAccounts" in ractive) ractive.fetchAccounts();
        } else if (
          newValue.orderItemFields[idx].type == "contact" &&
          $("datalist#contacts").length == 0
        ) {
          if ("fetchContacts" in ractive) ractive.fetchContacts();
        }
  ...
  ractive.observe("searchTerm", function (newValue, oldValue, keypath) {
    console.log(
      "'" + keypath + "' changed from '" + oldValue + "' to '" + newValue + "'",
    );
    let start = new Date().getTime();
...
Array.uniq = function (fieldName, arr) {
  ...
  for (var idx = 0; idx < arr.length; idx++) {
    if (
      index(arr[idx], fieldName) != undefined &&
...
function index(obj, keypath, value) {
  if (typeof keypath == "string") return index(obj, keypath.split("."), value);
  else if (keypath.length == 1 && value !== undefined)
    return (obj[keypath[0]] = value); // jshint ignore:line
  else if (keypath.length == 0) return obj;
  else return index(obj[keypath[0]], keypath.slice(1), value);
}
...
function parseDateIEPolyFill(timeString) {
  ...
  if (timeString.indexOf("-", timeString.indexOf("T")) != -1) {
    offset =
      timeString.substr(timeString.indexOf("-", timeString.indexOf("T")), 3) +
      ":" +
      timeString.substr(
        timeString.indexOf("-", timeString.indexOf("T")) + 3,
        2,
      );
  } else if (timeString.indexOf("+") != -1) {
    offset =
      timeString.substr(timeString.indexOf("+"), 3) +
      ":" +
      timeString.substr(timeString.indexOf("+") + 3, 2);
  }
