...
import imgAjaxLoader from '../images/ajax-loader.gif';
import imgGreyscaleIcon from '../images/icon/omny-icon-greyscale.png';

import  { pHelpModal } from '../partials/help-modal.html';

var fadeOutMessages = true;

export var BaseRactive = Ractive.extend({ // jshint ignore:line
  EASING_DURATION: 500,
  FF: {
...
//          console.log('partial '+d.url+' response: '+response);
  ...
  try {
    ...
    // JSON is available but keycloak does not load, perhaps race condition?
    // ractive.keycloak = Keycloak(keycloakConfig);
    ...
    console.error('Cannot init from '+keycloakConfig);
    console.error('Cannot init from '+JSON.stringify(keycloakConfig));
  ...
  document.querySelector('img.ajax-loader').src = imgAjaxLoader;
  ...
  // TODO partial not loaded yet...
  // document.querySelector('.powered-by-icon img').src = imgGreyscaleIcon;
...
//    document.getElementById('messages').scrollIntoView();
  ...
  showSearchMatched: function() {
    if (this === window) {
      console.error('TODO, why "this" not ractive here?');
      return;
    }
    var ractive = this;
    var entityName = ractive.get('entityName');
