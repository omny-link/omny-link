<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:element name="svg">
      ...
      <defs>
        ...
        <marker id="arrow1" markerWidth="13" markerHeight="13" refX="2" refY="7"
            orient="auto">
			    <path d="M2,2 L2,13 L8,7 L2,2" style="fill: #000000;" />
			  </marker>
        <marker id="arrow2" markerWidth="10" markerHeight="10" refx="0" refy="0" 
      ...
      <!-- LANES -->
      <xsl:apply-templates select="//bpmn:participant"/>
      ...
      <!-- FLOW OBJECTS -->
      <xsl:apply-templates select="//bpmn:messageFlow"/>
      <xsl:apply-templates select="//bpmn:sequenceFlow"/>

      <!-- GATEWAYS -->
      <xsl:apply-templates select="//bpmn:complexGateway"/>
      <xsl:apply-templates select="//bpmn:eventBasedGateway"/>
      ...
      <xsl:apply-templates select="//bpmn:receiveTask"/>
      <xsl:apply-templates select="//bpmn:scriptTask"/>
      <xsl:apply-templates select="//bpmn:sendTask"/>
      <xsl:apply-templates select="//bpmn:serviceTask"/>
      <xsl:apply-templates select="//bpmn:task"/>
      <xsl:apply-templates select="//bpmn:userTask"/>

      <!-- EVENTS -->
      <xsl:apply-templates select="//bpmn:startEvent"/>
      <xsl:apply-templates select="//bpmn:boundaryEvent"/>
      <xsl:apply-templates select="//bpmn:intermediateCatchEvent"/>
      <xsl:apply-templates select="//bpmn:intermediateThrowEvent"/>
      <xsl:apply-templates select="//bpmn:endEvent"/>

      <!-- DATA OBJECTS, after all flow lines have been drawn -->
      <xsl:apply-templates select="//bpmn:dataInput"/>
      <xsl:apply-templates select="//bpmn:dataObject"/>
      <xsl:apply-templates select="//bpmn:dataOutput"/>
      <xsl:apply-templates select="//bpmn:dataObjectReference"/>

      <xsl:apply-templates select="//bpmn:dataStore"/>
  ...
  <xsl:template match="bpmn:dataStoreReference">
    ...
    <xsl:if test="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@x">
      <xsl:element name="g">
        ...
        <xsl:attribute name="transform">translate(<xsl:value-of select="number(//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@x)-10"/>,<xsl:value-of select="$y"/>) scale(0.035)</xsl:attribute>
        <xsl:element name="path">
          <xsl:attribute name="d">
  ...
  <xsl:template match="bpmn:startEvent|bpmn:boundaryEvent|bpmn:intermediateCatchEvent|bpmn:intermediateThrowEvent|bpmn:endEvent">
    ...
    <xsl:element name="circle">
      ...
      <xsl:attribute name="fill">#fff</xsl:attribute>
      <xsl:attribute name="r">16</xsl:attribute>
    ...
    <xsl:choose>
      ...
      <xsl:when test="bpmn:messageEventDefinition">
        <xsl:call-template name="eventIcon">
          ...
          <xsl:with-param name="icon" select="$messageEventIcon"/>
          <xsl:with-param name="id" select="$id"/>
          <xsl:with-param name="bpmnElement" select="."/>
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="bpmn:signalEventDefinition">
        <xsl:call-template name="eventIcon">
          ...
          <xsl:with-param name="icon" select="$signalEventIcon"/>
          <xsl:with-param name="id" select="$id"/>
          <xsl:with-param name="bpmnElement" select="."/>
        </xsl:call-template>
      </xsl:when>
      ...
      <xsl:element name="path">
        <xsl:attribute name="d">
          <xsl:choose>
            ...
            <xsl:when test="local-name(.)='exclusiveGateway'">
  ...
  <xsl:template match="bpmn:participant">
    ...
    <xsl:element name="rect">
      ...
      <xsl:attribute name="fill">#999</xsl:attribute>
      <xsl:attribute name="height">
        <xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@height"/>
      </xsl:attribute>
    ...
    <xsl:comment>
      <xsl:text>Participant dimensions: </xsl:text>
      <xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@x"/>,
    ...
    <xsl:comment>
      <xsl:text>Participant label dimensions: </xsl:text>
  ...
  <xsl:template match="bpmn:lane">
    ...
    <xsl:element name="rect">
      <xsl:attribute name="class">lane</xsl:attribute>
      <xsl:attribute name="fill">#ccc</xsl:attribute>
      <xsl:attribute name="height">
        <xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@height"/>
      </xsl:attribute>
  ...
  <!-- TASKS -->
  <xsl:template match="bpmn:callActivity|bpmn:subProcess|bpmn:businessRuleTask|bpmn:receiveTask|bpmn:scriptTask|bpmn:sendTask|bpmn:serviceTask|bpmn:task|bpmn:userTask">
    <xsl:variable name="id" select="@id"/>
    <xsl:variable name="bpmnElement" select="//bpmndi:BPMNShape[@bpmnElement=$id]"/>
  ...
  <xsl:template name="flowOrAssociation">
    ...
    <xsl:element name="path">
      ...
      <xsl:attribute name="d">
        ...
        <xsl:choose>
          <!-- LEAVE LEFT with allowance of couple of pixels difference
          <xsl:when test="($srcElement/dc:Bounds/@x - $trgtElement/dc:Bounds/@x &lt; 5) and ($srcElement/dc:Bounds/@y &lt; $trgtElement/dc:Bounds/@y)">
          -->
          <xsl:when test="false">
            <xsl:value-of select="$srcElement/dc:Bounds/@x"/>
            <xsl:text> </xsl:text>
            <xsl:value-of select="$srcElement/dc:Bounds/@y"/>
          </xsl:when>
          <xsl:when test="false"><!-- LEAVE BOTTOM -->
            <xsl:value-of select="$srcElement/dc:Bounds/@x"/>
            <xsl:text> </xsl:text>
            <xsl:value-of select="$srcElement/dc:Bounds/@y+$srcElement/dc:Bounds/@height"/>
          </xsl:when>
          <xsl:when test="false"><!-- LEAVE TOP -->
            <xsl:value-of select="$srcElement/dc:Bounds/@x+($srcElement/dc:Bounds/@width div 2)"/>
            <xsl:text> </xsl:text>
            <xsl:value-of select="$srcElement/dc:Bounds/@y"/>
          </xsl:when>
          <xsl:otherwise><!-- LEAVE RIGHT -->
            <xsl:value-of select="$srcElement/dc:Bounds/@x+$srcElement/dc:Bounds/@width"/>
            <xsl:text> </xsl:text>
            <xsl:value-of select="$srcElement/dc:Bounds/@y+($srcElement/dc:Bounds/@height div 2)"/>
          </xsl:otherwise>          
        </xsl:choose>
  ...
  <xsl:template name="label">
    ...
    <xsl:apply-templates select="//bpmndi:BPMNShape[@bpmnElement=$id]/bpmndi:BPMNLabel/dc:Bounds">
    ...
    <xsl:variable name="t">
      <xsl:choose>
        <xsl:when test="local-name($bpmnElement)='startEvent'">
          <xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@x+4"/>,<xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@y+2"/>
        </xsl:when>
        <xsl:when test="local-name($bpmnElement)='endEvent'">
          <xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@x+5"/>,<xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@y+3"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@x+5"/>,<xsl:value-of select="//bpmndi:BPMNShape[@bpmnElement=$id]/dc:Bounds/@y+3"/>
        </xsl:otherwise>
  ...
  <xsl:variable name="dataStoreIcon">
    m 1000.0449,-608.55273 c -157.81266,0 -315.36613,13.01049 -438.7617,39.92187 -61.69778,13.45569 -114.83321,30.18084 -156.49023,52.13281 -40.12412,21.14419 -72.77527,48.41535 -82.84375,87.82617 -0.69331,2.05845 -1.19351,4.17696 -1.49414,6.32813 l -1.08594,5.26953 0.73633,3.57031 c -0.51535,315.041281 0,614.82949 0,938.8711 l 0.7207,3.49609 c 8.633,41.89172 42.30977,70.4074 83.9668,92.35938 41.65702,21.95198 94.79245,38.67516 156.49023,52.13086 123.39557,26.91138 280.94904,39.92382 438.7617,39.92382 157.8127,0 315.3662,-13.01244 438.7617,-39.92382 61.6978,-13.4557 114.8333,-30.17888 156.4903,-52.13086 41.657,-21.95198 75.3357,-50.46766 83.9687,-92.35938 l 0.7188,-3.49609 c -0,-309.85815 0.01,-647.33319 0,-940.84766 0.096,-1.4965 0.096,-2.99764 0,-4.49414 l 0,-0.66797 -0.1192,-0.57422 c -0.3037,-2.86398 -0.9597,-5.67953 -1.9531,-8.38281 -10.3403,-38.91405 -42.8049,-65.9196 -82.6152,-86.89844 -41.657,-21.95197 -94.7925,-38.67712 -156.4903,-52.13281 -123.3955,-26.91138 -280.949,-39.92187 -438.7617,-39.92187 z m 0,70 c 153.9084,0 308.076,13.06619 423.8457,38.31445 57.8849,12.62413 106.1932,28.50022 138.7715,45.66797 26.0001,13.70124 39.6105,27.31335 45.1797,37.49609 -5.5695,10.18265 -19.1804,23.79528 -45.1797,37.4961 -32.5783,17.16775 -80.8866,33.04188 -138.7715,45.66601 -115.7697,25.24826 -269.9373,38.31641 -423.8457,38.31641 -153.90834,0 -308.07597,-13.06815 -423.84568,-38.31641 -57.88486,-12.62413 -106.19322,-28.49826 -138.77149,-45.66601 -25.99925,-13.70082 -39.61016,-27.31345 -45.17968,-37.4961 5.56919,-10.18274 19.17961,-23.79485 45.17968,-37.49609 32.57