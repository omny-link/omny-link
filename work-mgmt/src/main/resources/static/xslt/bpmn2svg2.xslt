...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="showBounds">true</xsl:variable>
  ...
  <xsl:template name="label">
    ...
    <xsl:variable name="cutAfter" select="contains(substring($text,($len) div 2),' ')*(string-length(substring-before(substring($text,($len) div 2),' ')+1))"/>
    ...
    <xsl:comment>Calculate label bounds</xsl:comment>
    ...
    <xsl:variable name="labelX">
      <xsl:choose>
        ...
        <xsl:when test="contains(local-name(.),'callActivity') or contains(local-name(.),'subProcess') or contains(local-name(.),'task') or contains(local-name(.),'Task')">
          <xsl:comment>Use task element bounds for label x bound</xsl:comment>
        ...
        <xsl:otherwise>
          <xsl:comment>Use element bounds for label x bound</xsl:comment>
    ...
    <xsl:variable name="labelY">
      <xsl:choose>
        <xsl:when test="$diElement/bpmndi:BPMNLabel/dc:Bounds">
          <xsl:comment>Use task element bounds for label y bound</xsl:comment>
        ...
        <xsl:otherwise>
          <xsl:comment>Use element bounds for label y bound</xsl:comment>
    ...
    <xsl:if test="$debug='true'">
      <xsl:comment>
        ...
        <xsl:text>&#10;len div 2: </xsl:text><xsl:value-of select="$len div 2"/>
        <xsl:text>&#10;cutBefore: </xsl:text><xsl:value-of select="$cutBefore"/>
        <xsl:text>&#10;cutAfter: </xsl:text><xsl:value-of select="$cutAfter"/>
        <xsl:text>&#10;lenBefore: </xsl:text><xsl:value-of select="$lenBefore"/>
        <xsl:text>&#10;lenAfter: </xsl:text><xsl:value-of select="$lenAfter"/>
        <xsl:text>&#10;text: </xsl:text><xsl:value-of select="$text"/>
        <xsl:text>&#10;contains new line?: </xsl:text><xsl:value-of select="not(contains($text,'&#10;'))"/>
        <xsl:text>&#10;cutBefore suggests cut?: </xsl:text><xsl:value-of select="$cutBefore!=0"/>
        <xsl:text>&#10;cutAfter suggests cut?: </xsl:text><xsl:value-of select="$cutAfter>=0"/>
        <xsl:text>&#10;label height suggests cut?: </xsl:text><xsl:value-of select="number($diElement/bpmndi:BPMNLabel/dc:Bounds/@height) &gt;= (5*$fontSize)"/>
        <xsl:text>&#10;will cut?: </xsl:text><xsl:value-of select="not(contains($text,'&#10;')) and $cutBefore!=0 and $cutAfter!=0 and number($diElement/bpmndi:BPMNLabel/dc:Bounds/@height) &gt;= (5*$fontSize)"/>
        <xsl:text>&#10;line 1: </xsl:text><xsl:value-of select="substring($text,0,($len div 2) + $cutAfter)"/>
        <xsl:text>&#10;line 2: </xsl:text><xsl:value-of select="substring($text,($len div 2) + $cutAfter)"/>
      </xsl:comment>
    </xsl:if>
    ...
    <xsl:choose>
      <xsl:when test="not(contains($text,'&#10;')) and $cutBefore!=0 and $cutAfter!=0 and number($diElement/bpmndi:BPMNLabel/dc:Bounds/@height) &gt;= (2.75*$fontSize)">
      <!-- xsl:when test="1=2"-->
        <xsl:element name="text">
          <xsl:attribute name="class">label <xsl:value-of select="local-name(.)"/></xsl:attribute>
  ...
  <xsl:template name="multiLineText">
    ...
    <xsl:comment>Calc multi-line labels</xsl:comment>
